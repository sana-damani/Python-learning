...
# test for group collisions
def group_collide(group, other_object):
    collision = False
    for object in set(group):
        if object.collide(other_object):
            explosion = Sprite(object.get_pos(), [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound)
...
def click(pos):
    ...
    if (not started) and inwidth and inheight:
        ...
        explosion_group = set([])
        vel_factor = 0.6
        my_ship = Ship(INIT_POS, [0, 0], 0, ship_image, ship_info)
        soundtrack.rewind()
        soundtrack.play()
...
def draw(canvas):
    ...
    if lives == 0:
        started = False       
    # reduce lives if ship collides with rock
    elif group_collide(rock_group, my_ship):
        lives -= 1    
    
    # update score for each missile that collides with rock
    score += group_group_collide(missile_group, rock_group)

    # increase difficulty
    vel_factor = 0.6 + score / 10
...
def rock_spawner():
    global rock_group
    # limit num rocks to 12
    if len(rock_group) < 12 and started:
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        rock_vel = [random.random() * vel_factor - .3, random.random() * vel_factor - .3]
